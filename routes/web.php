...
$buildCategoryPayload = static function (Request $request, Project $project, string $category): array {
 ...
 *     nextArticle: ?Article,
 *     sameCategoryArticles: Collection<int, Article>,
 *     sameThemeArticles: Collection<int, Article>
 * }
 */
$buildArticlePayload = static function (Project $project, string $article): array {
...
Route::get('/blogs/{project:slug}', function (Request $request, Project $project) use ($buildBlogIndexPayload) {
    if ($project->isPrimaryPublicProject()) {
        return redirect()->to($project->publicIndexUrl($request->query()), 301);
    }
...
Route::get('/blogs/{project:slug}/categories/{category}', function (Request $request, Project $project, string $category) use ($buildCategoryPayload) {
    if ($project->isPrimaryPublicProject()) {
        return redirect()->to($project->publicCategoryUrl($category, $request->query()), 301);
    }
...
})->name('blogs.primary.article.og-image');

Route::get('/blogs/{project:slug}/articles/{article}/social-image.svg', function (Project $project, string $article) {
    if ($project->isPrimaryPublicProject()) {
        return redirect()->to($project->publicArticleSocialImageUrl($article, request()->query()), 301);
    }
...
Route::get('/blogs/{project:slug}/articles/{article}', function (Project $project, string $article) use ($buildArticlePayload) {
    if ($project->isPrimaryPublicProject()) {
        return redirect()->to($project->publicArticleUrl($article, request()->query()), 301);
    }
