<section class="grid gap-8 xl:grid-cols-[0.95fr_1.05fr]">
    @if ($this->centralProject)
        <div class="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-zinc-950/5 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/85">
            <p class="text-sm font-semibold uppercase tracking-[0.25em] text-lime-700 dark:text-lime-300">Projeto central</p>
            <h1 class="mt-3 text-4xl font-semibold tracking-tight text-zinc-950 dark:text-white">{{ $this->centralProject->name }}</h1>
            <p class="mt-2 text-sm leading-6 text-zinc-500 dark:text-zinc-400">O produto agora opera com um blog principal. Use este projeto para editar posicionamento, SEO, analytics e operacao editorial.</p>

            <div class="mt-6 rounded-3xl border border-zinc-200 bg-white/70 p-5 dark:border-zinc-800 dark:bg-zinc-950/40">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="rounded-full bg-zinc-100 px-3 py-1 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">{{ $this->centralProject->niche }}</span>
                    @if ($this->centralProject->target_location)
                        <span class="rounded-full bg-zinc-100 px-3 py-1 text-xs text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">{{ $this->centralProject->target_location }}</span>
                    @endif
                </div>

                <p class="mt-4 text-sm leading-7 text-zinc-600 dark:text-zinc-300">{{ $this->centralProject->description ?: 'Abra o projeto para configurar hero, analytics, CTA, Search Console e automacoes.' }}</p>

                <div class="mt-5 flex flex-wrap gap-2 text-xs text-zinc-600 dark:text-zinc-300">
                    <span class="rounded-full bg-zinc-100 px-3 py-1 dark:bg-zinc-800">{{ $this->centralProject->articles()->count() }} artigos</span>
                    <span class="rounded-full bg-zinc-100 px-3 py-1 dark:bg-zinc-800">{{ $this->centralProject->pillars()->count() }} pilares</span>
                    <span class="rounded-full bg-zinc-100 px-3 py-1 dark:bg-zinc-800">{{ $this->centralProject->clusters()->count() }} clusters</span>
                </div>

                <div class="mt-6 flex flex-wrap gap-3">
                    <flux:button :href="route('projects.show', $this->centralProject)" wire:navigate variant="primary">Editar projeto</flux:button>
                    <flux:button :href="$this->centralProject->publicIndexUrl()" target="_blank" variant="ghost">Ver blog publico</flux:button>
                </div>
            </div>
        </div>
    @else
        <div class="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-zinc-950/5 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/85">
            <p class="text-sm font-semibold uppercase tracking-[0.25em] text-lime-700 dark:text-lime-300">Projeto central</p>
            <h1 class="mt-3 text-4xl font-semibold tracking-tight text-zinc-950 dark:text-white">Criar o blog principal</h1>
    ...
    <div class="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-zinc-950/5 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/85">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400">Historico e referencia do projeto central do blog.</p>
            </div>
        </div>
